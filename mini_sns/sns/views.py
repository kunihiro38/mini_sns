...
def add(request):
    ...
    # ゼロより大きければ既に登録済み
    if frd_num > 0:
        message.info(request, add_user.username + " は既に追加されています。")
        return redirect(to='/sns')

    # ここからFriendの登録処理
    frd = Friend()
    frd.owner = reques.user
    frd.user = add_user
...
def get_your_group_message(owner, glist, find):
    ...
    if find==None:
        ...
        messges = Message.objects.filter(Q(group__in=groups)|Q(group__in=me_groups)).filter(content__contains=find)[:100]
    return messages
